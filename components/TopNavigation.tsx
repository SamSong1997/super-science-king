...
import { BookOpen, FlaskConical, Gamepad2, GraduationCap, MessageSquare, ScrollText, Home, User } from 'lucide-react'
              ...
              {navigationItems.map((item) => {
                ...
                return (
                  <Link key={item.href} href={item.href}>
                    <Button
                      variant="ghost"
                      className={`flex items-center gap-1.5 px-3 py-2 rounded-lg transition-colors h-9 ${
                        isActive
          ...
          <div className="hidden md:flex items-center shrink-0">
            <div className="relative">
              <button
                ...
                className="flex flex-row items-center gap-2 px-3 py-2 rounded-lg hover:bg-[#e8e6dc] transition-colors"
              >
                <div className="w-8 h-8 bg-gradient-to-br from-[#d97757] to-[#6a9bcc] rounded-full flex items-center justify-center text-lg shrink-0">
                  {USER_DATA.avatar}
                </div>
                <div className="flex flex-col text-left">
                  <div className="text-sm font-medium text-[#141413] whitespace-nowrap">
                    {USER_DATA.name}
                  </div>
                  <div className="text-xs text-[#b0aea5] whitespace-nowrap">Lv.{USER_DATA.level}</div>
                </div>
              </button>
